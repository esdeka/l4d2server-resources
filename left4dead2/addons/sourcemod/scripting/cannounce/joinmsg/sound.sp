...
Action Command_SetJoinSnd(int client, int args)
{
	char target[65];
...
Action Command_SetJoinSndID(int client, int args)
{
	char steamId[24];
	char sndFile[SOUNDFILE_PATH_LEN];
	int charsSet;
...
Action Command_PlaySnd(int client, int args)
{
	char sFile[256];
...
bool SetJoinSnd(char[] steamId, char[] sndFile)
{
	...
	{
		hKVCustomJoinMessages.SetString("soundfile", sndFile);
...
void LoadSoundFilesCustomPlayer()
{
	...
	hKVCustomJoinMessages.GotoFirstSubKey();
	//cycle thru soundfile values, if they exist, add to download table and precache
	do
	{
		hKVCustomJoinMessages.GetString("soundfile", sndFile, sizeof(sndFile));
		if (strlen(sndFile) > 0)
		{
	...
	while (hKVCustomJoinMessages.GotoNextKey());
	hKVCustomJoinMessages.Rewind();
}